...
function exportNationProfiles(){
	var wrap = document.getElementById("export-nation-profiles");
		wrap.className = "c-fix";

	var root_url = "https://www.brookings.edu/wp-content/uploads/2018/04/";
	...
	//var prompt = select.appendChild(document.createElement("option"));
	//prompt.innerHTML = "Select a profile";
	//prompt.setAttribute("disabled","yes");
	//prompt.setAttribute("selected","yes");
	...
	var appendOption = function(d, subfolder, optgroup){
		...
		var fname = d.file.toLowerCase().replace(/\s/g,"-");
		o.value = encodeURI(root_url + fname);
		o.innerHTML = d.name;
		return o.value;
	}
